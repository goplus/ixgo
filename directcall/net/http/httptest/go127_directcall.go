...
func init() {
	ixgo.RegisterDirectCalls("net/http/httptest", map[string]ixgo.DirectCallAdapter{
		...
		"NewTestServer":                    func_NewTestServer,
...
func func_NewTestServer(ctx ixgo.DirectCallContext) {
	ctx.SetResult(q.NewTestServer(ixgo.DirectCallArg[testing.TB](ctx, 0), ixgo.DirectCallArg[http.Handler](ctx, 1)))
}
