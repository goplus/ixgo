...
func func_Read(ctx ixgo.DirectCallContext) {
	ctx.SetResult(q.Read(ixgo.DirectCallArg[io.Reader](ctx, 0), ixgo.DirectCallArg[q.ByteOrder](ctx, 1), ixgo.DirectCallArg[any](ctx, 2)))
}

func func_Size(ctx ixgo.DirectCallContext) {
	ctx.SetResult(q.Size(ixgo.DirectCallArg[any](ctx, 0)))
}

func func_Write(ctx ixgo.DirectCallContext) {
	ctx.SetResult(q.Write(ixgo.DirectCallArg[io.Writer](ctx, 0), ixgo.DirectCallArg[q.ByteOrder](ctx, 1), ixgo.DirectCallArg[any](ctx, 2)))
}
