...
func init() {
	ixgo.RegisterDirectCalls("encoding/base64", map[string]ixgo.DirectCallAdapter{
...
func method_Encoding_Strict(ctx ixgo.DirectCallContext) {
	ctx.SetResult(q.Encoding.Strict(ixgo.DirectCallArg[q.Encoding](ctx, 0)))
}

func method_ptr_Encoding_Strict(ctx ixgo.DirectCallContext) {
	ctx.SetResult((*q.Encoding).Strict(ixgo.DirectCallArg[*q.Encoding](ctx, 0)))
}
