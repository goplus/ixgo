...
func func_Check(ctx ixgo.DirectCallContext) {
	ctx.SetResult(q.Check(ixgo.DirectCallArg[interface{}](ctx, 0), ixgo.DirectCallArg[*q.Config](ctx, 1)))
}

func func_CheckEqual(ctx ixgo.DirectCallContext) {
	ctx.SetResult(q.CheckEqual(ixgo.DirectCallArg[interface{}](ctx, 0), ixgo.DirectCallArg[interface{}](ctx, 1), ixgo.DirectCallArg[*q.Config](ctx, 2)))
}
