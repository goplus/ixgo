...
func method_ptr_Template_Execute(ctx ixgo.DirectCallContext) {
	ctx.SetResult((*q.Template).Execute(ixgo.DirectCallArg[*q.Template](ctx, 0), ixgo.DirectCallArg[io.Writer](ctx, 1), ixgo.DirectCallArg[interface{}](ctx, 2)))
}

func method_ptr_Template_ExecuteTemplate(ctx ixgo.DirectCallContext) {
	ctx.SetResult((*q.Template).ExecuteTemplate(ixgo.DirectCallArg[*q.Template](ctx, 0), ixgo.DirectCallArg[io.Writer](ctx, 1), ixgo.DirectCallArg[string](ctx, 2), ixgo.DirectCallArg[interface{}](ctx, 3)))
}
