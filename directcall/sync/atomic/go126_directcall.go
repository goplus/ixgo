...
func method_ptr_Value_CompareAndSwap(ctx ixgo.DirectCallContext) {
	ctx.SetResult((*q.Value).CompareAndSwap(ixgo.DirectCallArg[*q.Value](ctx, 0), ixgo.DirectCallArg[any](ctx, 1), ixgo.DirectCallArg[any](ctx, 2)))
}
...
func method_ptr_Value_Store(ctx ixgo.DirectCallContext) {
	(*q.Value).Store(ixgo.DirectCallArg[*q.Value](ctx, 0), ixgo.DirectCallArg[any](ctx, 1))
}

func method_ptr_Value_Swap(ctx ixgo.DirectCallContext) {
	ctx.SetResult((*q.Value).Swap(ixgo.DirectCallArg[*q.Value](ctx, 0), ixgo.DirectCallArg[any](ctx, 1)))
}
