...
func init() {
	ixgo.RegisterDirectCalls("go/ast", map[string]ixgo.DirectCallAdapter{
		...
		"(*Directive).End":       method_ptr_Directive_End,
		"(*Directive).Pos":       method_ptr_Directive_Pos,
...
func method_ptr_Directive_End(ctx ixgo.DirectCallContext) {
	ctx.SetResult((*q.Directive).End(ixgo.DirectCallArg[*q.Directive](ctx, 0)))
}

func method_ptr_Directive_Pos(ctx ixgo.DirectCallContext) {
	ctx.SetResult((*q.Directive).Pos(ixgo.DirectCallArg[*q.Directive](ctx, 0)))
}
...
func func_Fprint(ctx ixgo.DirectCallContext) {
	ctx.SetResult(q.Fprint(ixgo.DirectCallArg[io.Writer](ctx, 0), ixgo.DirectCallArg[*token.FileSet](ctx, 1), ixgo.DirectCallArg[interface{}](ctx, 2), ixgo.DirectCallArg[q.FieldFilter](ctx, 3)))
}
...
func func_Print(ctx ixgo.DirectCallContext) {
	ctx.SetResult(q.Print(ixgo.DirectCallArg[*token.FileSet](ctx, 0), ixgo.DirectCallArg[interface{}](ctx, 1)))
}
