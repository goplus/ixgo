...
func init() {
	ixgo.RegisterDirectCalls("os", map[string]ixgo.DirectCallAdapter{
		...
		"(*Process).WithHandle":      method_ptr_Process_WithHandle,
...
func method_ptr_Process_WithHandle(ctx ixgo.DirectCallContext) {
	ctx.SetResult((*q.Process).WithHandle(ixgo.DirectCallArg[*q.Process](ctx, 0), ixgo.DirectCallArg[func(handle uintptr)](ctx, 1)))
}
