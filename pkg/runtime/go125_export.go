...
func init() {
	ixgo.RegisterPackageLazy("runtime", func() *ixgo.Package {
		return &ixgo.Package{
			Name: "runtime",
			Path: "runtime",
			Deps: map[string]string{
				...
				"internal/runtime/strconv":  "strconv",
			...
			Funcs: map[string]reflect.Value{
				...
				"SetDefaultGOMAXPROCS":    reflect.ValueOf(q.SetDefaultGOMAXPROCS),
			...
			TypedConsts: map[string]ixgo.TypedConst{
				"GOARCH": {Typ: reflect.TypeOf(q.GOARCH), Value: constant.MakeString(string(q.GOARCH))},
				"GOOS":   {Typ: reflect.TypeOf(q.GOOS), Value: constant.MakeString(string(q.GOOS))},
			},
